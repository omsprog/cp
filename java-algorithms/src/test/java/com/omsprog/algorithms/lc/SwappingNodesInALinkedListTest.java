package com.omsprog.algorithms.lc;

import com.omsprog.algorithms.util.TestUtil;
import org.junit.jupiter.api.Test;

import static org.junit.jupiter.api.Assertions.*;
class SwappingNodesInALinkedListTest {
    @Test
    void test() {
        SwappingNodesInALinkedList sN = new SwappingNodesInALinkedList();
        assertTrue(TestUtil.isLinkedListValid(
                new int[] {1,4,3,2,5}, sN.swapNodes(TestUtil.buildLinkedList(new int[] {1,2,3,4,5}), 2)
        ));

        assertTrue(TestUtil.isLinkedListValid(
                new int[] {7,9,6,6,8,7,3,0,9,5}, sN.swapNodes(TestUtil.buildLinkedList(new int[] {7,9,6,6,7,8,3,0,9,5}), 5)
        ));
    }
}